...
function WorkflowDetailController($scope, $mdDialog, mediator, $stateParams, workflowMediatorService, $q, $timeout) {
  ...
  function showDeleteDialog(workorders, event) {
    var workorder = workorders.filter(function(workorder) {
      return String(workorder.workflowId) === String(self.workflow.id);
    });
    var title = (workorder.length)
      ? "Workflow is used at least by at least 1 workorder, are you sure you want to delete workflow #'"+self.workflow.id+"?"
      : "Would you like to delete workflow #"+self.workflow.id+"?";
  ...
  self.deleteStep = function(event, step, workflow) {
    ...
    showDeleteStepDialog(step, event).then(function() {
      ...
      workflowMediatorService.updateWorkflow(workflow).then(function(_workflow) {
