...
function WorkflowMediatorService(mediator, config) {
  ...
  this.appformsFormsTopics = new MediatorTopicUtility(mediator).prefix(CONSTANTS.APPFORMS_PREFIX).entity(CONSTANTS.FORMS_ENTITY_NAME);
...
/**
 *
 * Listing all forms. This is used to assign forms to a workflow step.
 *
 * @returns {Promise}
 */
WorkflowMediatorService.prototype.listForms = function listForms() {
  var promise = this.getErrorAndDoneTopicPromises(this.appformsFormsTopics, CONSTANTS.TOPICS.LIST);

  this.mediator.publish(this.appformsFormsTopics.getTopic(CONSTANTS.TOPICS.LIST));
...
WorkflowMediatorService.prototype.removeWorkflow = function removeWorkorder(workflowToRemove) {
  ...
  var promise = this.getErrorAndDoneTopicPromises(this.workflowsTopics, CONSTANTS.TOPICS.REMOVE, workflowToRemove.id);
